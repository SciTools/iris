...
def _mask_by_path(data, path, dims):
    if type(data) is not np.ma.masked_array:
        data = np.ma.masked_array(data, mask=False)

    poly_mask = np.empty(np.array(data.shape)[np.array(dims)])
    for i in np.ndindex(poly_mask.shape):
        poly_mask[i] = path.contains_point(i)
    new_mask = data.mask | (broadcast_to_shape(poly_mask,
                                               data.shape,
                                               dims) == 1.0)
    data.mask = new_mask
...
class CubeExplorer(object):
    ...
    def add_picker(self, plot_func, *args, **kwargs):
        ...
        up plot by ctrl+clicking on the cube explorer plot.
        ...
        * hook:
        ...
        def _on_pick(event):
            if event.artist.get_axes() != self.ax:
                return
            if event.mouseevent.key is 'control':
    ...
    def add_poly_picker(self, *args, **kwargs):
        """
        ...
        if not hasattr(self, '_poly_verts'):
            "Adding poly_verts"
            self._poly_verts = []

        def _on_poly_pick(event):
            if event.artist.get_axes() != self.ax:
                return
            ...
            if event.mouseevent.dblclick:
                codes = [Path.LINETO] * len(self._poly_verts)
                codes[0] = Path.MOVETO
                codes[-1] = Path.CLOSEPOLY
                self.cube.data = _mask_by_path(self.cube.data,
                                               Path(self._poly_verts, codes),
                                               picker_plot_dims)

                self._refresh_plot()

        self.ax.figure.canvas.mpl_connect('pick_event', _on_poly_pick)
