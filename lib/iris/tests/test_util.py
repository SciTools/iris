...
class Test_CubeList_pad_coords(tests.IrisTest):
    ...
    def test_extract_one_str_dim(self):
        a, b = iris.util.pad_coords(iris.cube.CubeList([self.cube[2:],
                                   self.cube[:4]]), 'time')
    ...
    def test_extract_two_dims(self):
        a, b = iris.util.pad_coords(iris.cube.CubeList(
            [self.cube[2:, 20:, :],
             self.cube[:4, :-10, :]]), ['time', 'latitude'])
    ...
    def test_incompatible_dim_coords(self):
        with self.assertRaisesRegexp(ValueError, "latitude dim_coords "
            ...
            a, b = iris.util.pad_coords(iris.cube.CubeList([p, q]), 'latitude')
    ...
    def test_incompatible_aux_coords(self):
        with self.assertRaisesRegexp(ValueError,
            ...
            a, b = iris.util.pad_coords(iris.cube.CubeList([p, q]), 'time')
