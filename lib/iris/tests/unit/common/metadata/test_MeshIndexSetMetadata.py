...
class Test__eq__:
    ...
    def _setup(self, mocker):
        self.values = dict(
            ...
            location=mocker.sentinel.location,
    ...
    def test_op_lenient_same_none_nonmember(self, mocker):
        lmetadata = self.cls(**self.values)
    ...
    def test_op_lenient_different_nonmember(self, mocker):
        lmetadata = self.cls(**self.values)
        right = self.values.copy()
    ...
    def test_op_strict_different_nonmember(self, mocker):
        lmetadata = self.cls(**self.values)
        right = self.values.copy()
    ...
    def test_op_strict_different_nonmember_none(self, mocker):
        lmetadata = self.cls(**self.values)
        right = self.values.copy()
        right["long_name"] = None
        rmetadata = self.cls(**right)
...
class Test_combine:
    ...
    def test_op_lenient_same(self, mocker):
        ...
        mocker.patch("iris.common.metadata._LENIENT", return_value=True)
        assert self.values == lmetadata.combine(rmetadata)._asdict()
        assert self.values == rmetadata.combine(lmetadata)._asdict()
