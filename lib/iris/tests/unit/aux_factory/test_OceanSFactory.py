...
class Test___init__(tests.IrisTest):
    ...
    def test_insufficient_coordinates(self):
        with self.assertRaises(ValueError):
            OceanSFactory()
        with self.assertRaises(ValueError):
            OceanSFactory(s=None, eta=self.eta, depth=self.depth,
                          a=self.a, b=self.b, depth_c=self.depth_c)
        with self.assertRaises(ValueError):
            OceanSFactory(s=self.s, eta=None, depth=self.depth,
                          a=self.a, b=self.b, depth_c=self.depth_c)
        with self.assertRaises(ValueError):
            OceanSFactory(s=self.s, eta=self.eta, depth=None,
                          a=self.a, b=self.b, depth_c=self.depth_c)
        with self.assertRaises(ValueError):
            OceanSFactory(s=self.s, eta=self.eta, depth=self.depth,
                          a=None, b=self.b, depth_c=self.depth_c)
        with self.assertRaises(ValueError):
            OceanSFactory(s=self.s, eta=self.eta, depth=self.depth,
                          a=self.a, b=None, depth_c=self.depth_c)
        with self.assertRaises(ValueError):
            OceanSFactory(s=self.s, eta=self.eta, depth=self.depth,
                          a=self.a, b=self.b, depth_c=None)
...
class Test_make_coord(tests.IrisTest):
    ...
    def coord_dims(coord):
        mapping = dict(s=(0,), eta=(1, 2), depth=(1, 2),
                       a=(), b=(), depth_c=())
    ...
    def setUp(self):
        self.s = DimCoord(np.arange(-0.975, 0, 0.05, dtype=float), units='1',
                          long_name='s')
