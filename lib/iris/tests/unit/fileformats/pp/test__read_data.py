...
"""Unit tests for the `iris.fileformats.pp._read_data` function."""
...
class Test_read_data__land_packed(tests.IrisTest):
    def setUp(self):
        ...
        self.land_mask = pp._LandMask(mock.Mock(data=self.land))
    ...
    def test_no_land_mask(self):
        with mock.patch('numpy.fromfile',
                        return_value=np.arange(3)):
            with self.assertRaises(ValueError):
                pp._read_data(mock.Mock(), self.create_lbpack(120),
                              3 * pp.PP_WORD_DEPTH, (3, 4), np.dtype('>f4'),
                              -999, mask=None)
    ...
    def check_read_data(self, field_data, lbpack, mask):
        # Calls pp._read_data with the necessary mocked items, an lbpack
        # instance, the correct data length, shape and mask instance.
        with mock.patch('numpy.fromfile', return_value=field_data):
            return pp._read_data(mock.Mock(), self.create_lbpack(lbpack),
                                 len(field_data) * pp.PP_WORD_DEPTH,
