...
class TestIdentify:
    def test_no_coord_system_mappings_returns_empty(self, named_variable):
        ...
        crs_var = named_variable("crs_var")
    ...
    def test_valid_coord_ref_identified(self, named_variable):
        """Mapping with a real coordinate reference that exists is accepted."""
        ref_source = named_variable("ref_source")
        setattr(ref_source, CF_IDENTITY, "crs_var")
        crs_var = named_variable("crs_var")
        coord_var = named_variable("lat")
        vars_all = {
            "crs_var": crs_var,
            "lat": coord_var,
    ...
    def test_missing_mapping_variable_warns(self, named_variable, assert_warning_gated):
    ...
    def test_missing_coord_ref_warns(self, named_variable):
...
class TestIdentifyGroupingByCRS:
    ...
    def test_multiple_coords_one_cs(self, named_variable):
        """Two coordinates both mapping to the same CRS: one CRS identified."""
        ref_source = named_variable("ref_source")
        setattr(ref_source, "grid_mapping", "crs_1")
        crs_1 = named_variable("crs_1")
        lat = named_variable("lat")
        lon = named_variable("lon")
        vars_all = {
            "crs_1": crs_1,
            "lat": lat,
            "lon": lon,
    ...
    def test_partial_coords_missing_cs_still_identified(self, named_variable):
    ...
    def test_all_coords_missing_cs_excluded(self, named_variable):
