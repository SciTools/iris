...
class TestIdentify:
    def test_one_ref(self, named_variable):
        ...
        expected = {
            subject_name: CFAuxiliaryCoordinateVariable(subject_name, ref_subject)
        }
        result = CFAuxiliaryCoordinateVariable.identify(vars_all)
    ...
    def test_two_refs(self, named_variable):
        ...
        expected = {
            name: CFAuxiliaryCoordinateVariable(name, var)
    ...
    def test_ignore(self, named_variable):
        ...
        expected = {
            expected_name: CFAuxiliaryCoordinateVariable(
                expected_name, ref_subject_vars[expected_name]
            )
        }
        result = CFAuxiliaryCoordinateVariable.identify(
    ...
    def test_target_unknown_raises(self, named_variable):
        ...
        with pytest.raises(ValueError, match=message):
            CFAuxiliaryCoordinateVariable.identify(vars_all, target="unknown")
    ...
    def test_target_wrong_type_raises(self, named_variable):
        ...
        with pytest.raises(TypeError, match=message):
            CFAuxiliaryCoordinateVariable.identify(vars_all, target=object())
    ...
    def test_warn(self, named_variable, assert_warning_gated):
        ...
        def operation(warn: bool):
            ...
            result = CFAuxiliaryCoordinateVariable.identify(vars_all, warn=warn)
            assert {} == result

        warn_regex = (
            rf"Missing CF-netCDF auxiliary coordinate variable {subject_name!r}.*"
        )
