...
def _get_cf_var_data(cf_var, filename):
    ...
    if hasattr(cf_var, "_data_array"):
        ...
        if total_bytes < _LAZYVAR_MIN_BYTES:
            ...
            if CHUNK_CONTROL.mode == CHUNK_CONTROL.Modes.AS_DASK:
                return as_lazy_data(proxy, chunks=None, as_dask=True)
            chunks = cf_var.cf_data.chunking()
            #
            if (
                chunks is None
                and CHUNK_CONTROL.mode == CHUNK_CONTROL.Modes.FROM_FILE
            ):
                raise KeyError(
            ...
            if CHUNK_CONTROL.mode == CHUNK_CONTROL.Modes.FROM_FILE:
...
class ChunkControl(threading.local):
    def __init__(self, var_dim_chunksizes=None):
        ...
        self.Modes = Enum("Modes", ["DEFAULT", "FROM_FILE", "AS_DASK"])
