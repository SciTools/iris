...
def interpolate(cube, sample_points, method=None):
    ...
    * method
    ...
    for coord in cube.coords():
        # Don't create squished coords that are generated by a factory.
        if coord.name() in [f.name() for f in cube.aux_factories]:
            continue
        src_dims = cube.coord_dims(coord)
    ...
    for i in range(trajectory_size):
        ...
        for column_coord in column.coords():
            # Don't try to fill in coords that are generated by a factory.
            if column_coord.name() in [f.name() for f in cube.aux_factories]:
                continue
