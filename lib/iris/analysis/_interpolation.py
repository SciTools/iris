...
def _nearest_neighbour_indices_ndcoords(cube, sample_points, cache=None):
    ...
    if sample_points:
        try:
            ...
            raise ValueError('Sample points must be a list of '
    ...
    sample_space_coord_names = [coord.name() for coord in
                                sample_space_cube.coords()]
    new_order = [sample_space_coord_names.index(name) for name in
                 sample_point_coord_names]
    ...
    sample_space_coords = sample_space_cube.dim_coords + \
                          sample_space_cube.aux_coords
    ...
    if cache is not None and cube in cache:
        ...
        # Create a "sample space position" for each datum:
        # sample_space_data_positions[coord_index][datum_index]
        sample_space_data_positions = np.empty((len(
            sample_space_coords_and_dims), sample_space_cube.data.size),
            dtype=float)
        for d, ndi in enumerate(np.ndindex(sample_space_cube.data.shape)):
            for c, (coord, coord_dims) in enumerate(
                ...
                keys = tuple(ndi[ind] for ind in coord_dims) if coord_dims \
