...
def gen_gallery(app, doctree):
    ...
    link_template = '<a href="%(href)s"><img src="%(thumb_file)s" ' + \
                    'border="0" alt="%(alternative_text)s"/></a>'

    header_template = '<div class="section" id="%s"><h4>%s<a ' + \
                      'class="headerlink" href="#%s" title="Permalink to ' + \
                      'this headline">&para;</a></h4>'

    toc_template = '<li><a class="reference internal" href="#%s">%s</a></li>'
...
'''
    ...
    for subdir in dirs:
        ...
        for (subdir, basename, thumbfile) in data:
            if thumbfile is not None:
                ...
                link = 'examples/%s/%s.html#%s' % (subdir, basename, anchor)
                rows.append(link_template % {'href': link,
                                             'thumb_file': thumbfile,
                                             'alternative_text': basename})
                random_image.append(random_image_template % {
                    'link': link,
                    'thumbfile': thumbfile,
                    'basename': basename,
    ...
    random_image_content = '\n'.join([random_image_join % (i, line)
                                      for i, line in enumerate(random_image)])
    random_image_content = random_image_content_template % random_image_content
