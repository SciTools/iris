...
def main():
    ...
    ax2 = divider.new_vertical(size="5%", pad=0.5, axes_class=plt.Axes, pack_start=True) #creates 2nd axis
    fig.add_axes(ax2) # plots ax2
    plt.sca(ax2)#?
    cbar = plt.colorbar(im, cax=ax2, orientation='horizontal') # puts colour bar on second axis
    ...
    ax3 = divider.new_horizontal(size="30%", pad=0.4, axes_class=plt.Axes) # create 3rd axis
