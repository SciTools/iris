...
#. `@ESadek-MO`_, `@trexfeathers`_, `@bjlittle`_, `@HGWright`_, `@pp-mo`_,
   `@stephenworsley`_ and `@ukmo-ccbunney`_ converted the entirety of the tests
   from unittest to pytest. Iris is now also ruff-PT compliant, save for PT019.
   (:issue:`6212`, :pull:`6939`)

#. `@hsteptoe`_ and `@ESadek-MO`_ (reviewer) updated chained assignment usage within the tests
