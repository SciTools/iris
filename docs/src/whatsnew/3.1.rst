...
.. dropdown:: :opticon:`report` Release Highlights
   ...
   * Much faster import time, from minimised loading of submodules.
     Note however that this may break existing code which does not declare full imports.
...
#. The :ref:`adoption of 'isort' <isort_adopt>`  means that subpackage loading is everywhere minimised.

   However, this may break existing code that relied on subpackages being imported 'automatically'.
